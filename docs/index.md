
# О платформе

**Marketaut** — это платформа для удобного подключения ИИ-инструментов. 

![](main.png)



Она позволяет визуально строить автоматизации и подключать ИИ-агентов без написания кода, установки дополнительный приложений и запуска сложных команд.

[Домашняя страница проекта](https://marketaut.ru)

[Где найти документацию и как связаться с поддержкой](1-help/help.md)

[Инструкция по подключению за 10 минут](2-quick-start/quick_start.md)

[Регистрация и начало работы](3-start/register.md)

[Работа с платформой](4-working-with-platform/00-intro.md)

[Настройка ИИ агента](5-claude/01-claude-connect.md)