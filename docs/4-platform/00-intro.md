...
Одна диаграмма — одна автоматизация. Например, диаграмма может описывать подключение магазина Wildberries к  MCP-серверу.
...
Аккаунт настраивается один раз, после чего может использоваться в любых узлах, которым требуется доступ к соответствующему сервису. Это позволяет не вводить один и те же данные повторно в каждом модуле.
